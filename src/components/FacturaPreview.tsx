...
export default function FacturaPreview({ factura, mostrarStatus }: FacturaPreviewProps) {
  ...
  return (
    <div className="w-full max-w-[850px] mx-auto bg-white rounded-xl shadow p-8 relative overflow-hidden" style={{ minHeight: '1100px' }}>
      {/* Sello de pagado */}
      ...
      <div className="flex flex-col md:flex-row md:justify-between md:items-start mb-6 gap-4">
        <div className="flex flex-row items-center gap-4 w-full md:w-auto">
          {factura?.negocio?.logo_url && (
            <img src={factura.negocio.logo_url} alt="Logo" className="h-24 w-24 object-contain rounded bg-white border" />
          )}
          <div className="flex-1">
            <div className="font-bold text-2xl mb-1">{nombreNegocio}</div>
