...
interface GestionMensajesPredefinidosFacturaModalProps {
  ...
  onMensajeActualizado?: (status: TipoMensaje, canal: CanalMensaje, mensaje: string) => void; // Nuevo prop para comunicar cambios
}
  ...
  const guardarMensaje = async () => {
    ...
    try {
      ...
      if (onMensajeActualizado) {
        onMensajeActualizado(selectedStatus, selectedCanal, mensajeEditadoActual);
      }
  ...
  return (
    <Dialog open={open} onClose={onClose} className="relative z-50">
      <div className="fixed inset-0 bg-black/30" aria-hidden="true" />
             ...
             <div className="fixed inset-0 flex items-center justify-center p-2 lg:p-4">
         <Dialog.Panel className="mx-auto max-w-6xl w-full bg-white dark:bg-gray-800 rounded-xl shadow-2xl h-[95vh] lg:h-[90vh] flex flex-col overflow-hidden">
                     {/* Header */}
           <div className="flex items-center justify-between p-4 lg:p-6 border-b border-gray-200 dark:border-gray-700">
             <div className="flex items-center space-x-2 lg:space-x-3">
               <div className="p-1.5 lg:p-2 bg-blue-100 dark:bg-blue-900 rounded-lg">
                 <FiMessageSquare className="w-5 h-5 lg:w-6 lg:h-6 text-blue-600 dark:text-blue-400" />
               </div>
               <div className="min-w-0 flex-1">
                 <Dialog.Title className="text-lg lg:text-xl font-semibold text-gray-900 dark:text-white truncate">
                   Gestión de Mensajes Predefinidos Factura
                 </Dialog.Title>
                 <p className="text-xs lg:text-sm text-gray-500 dark:text-gray-400">
                   Personaliza los mensajes para cada estado de factura
                 </p>
               </div>
             </div>
             <button
               onClick={onClose}
               className="p-2 lg:p-2 text-gray-400 hover:text-gray-600 dark:hover:text-gray-300 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors flex-shrink-0 z-10 relative"
               title="Cerrar"
             >
               <XMarkIcon className="w-6 h-6 lg:w-6 lg:h-6" />
             </button>
           </div>
                 ...
                 <div className="grid grid-cols-2 lg:grid-cols-1 gap-2">
                   {STATUS_CONFIG.map((status) => (
                     <button
                       ...
                       className={`w-full p-3 lg:p-4 rounded-lg border transition-all duration-200 text-left ${
                     ...
                     >
                       <div className="flex items-center space-x-2 lg:space-x-3">
                         <div className={`p-1.5 lg:p-2 rounded-lg ${status.bgColor}`}>
                           <div className={`${status.color} w-4 h-4 lg:w-5 lg:h-5`}>{status.icon}</div>
                         </div>
                         <div className="min-w-0 flex-1">
                           <div className="font-medium text-gray-900 dark:text-white text-sm lg:text-base truncate">
                             {status.nombre}
                           </div>
                           <div className="text-xs lg:text-sm text-gray-500 dark:text-gray-400 hidden lg:block">
                             {status.descripcion}
                           </div>
                         </div>
                       </div>
             ...
             <div className="flex-1 flex flex-col min-h-0 overflow-hidden">
              {selectedStatus ? (
                <>
                     ...
                     <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
                       <div className="flex items-center space-x-3">
                         {(() => {
                           ...
                           return (
                             <>
                               <div className={`p-2 lg:p-3 rounded-lg ${config?.bgColor}`}>
                                 <div className={`${config?.color} w-5 h-5 lg:w-6 lg:h-6`}>{config?.icon}</div>
                               </div>
                               <div>
                     ...
                     <div className="flex space-x-2">
                       {canales.map((canal) => (
                         <button
                           ...
                           className={`px-3 lg:px-4 py-2 rounded-lg border transition-all duration-200 flex items-center space-x-1 lg:space-x-2 text-sm relative ${
                             selectedCanal === canal.id
                               ? 'border-blue-500 bg-blue-50 dark:bg-blue-900/20 text-blue-700 dark:text-blue-300'
                               : 'border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 text-gray-700 dark:text-gray-300 hover:border-gray-300 dark:hover:border-gray-600'
                           }`}
                         >
                           <span className="text-base lg:text-lg">{canal.icon}</span>
                     ...
                     <div className="h-full flex flex-col">
                       ...
                       <div className="flex flex-wrap gap-1 lg:gap-2 mb-4">
                         {variables.map((variable) => (
                           <button
                             ...
                             className="px-2 lg:px-3 py-1 bg-blue-100 dark:bg-blue-900 text-blue-700 dark:text-blue-300 rounded-lg text-xs lg:text-sm hover:bg-blue-200 dark:hover:bg-blue-800 transition-colors"
                       ...
                       <div className="flex-1 min-h-[200px] lg:min-h-0">
                         <textarea
                           ...
                           className="w-full h-full min-h-[200px] lg:min-h-0 p-3 lg:p-4 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-white resize-none focus:ring-2 focus:ring-blue-500 focus:border-transparent disabled:opacity-50 text-sm lg:text-base"
                   ...
                   <div className="lg:hidden p-4 border-t border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 flex-shrink-0">
                       ...
                       {mensajesPorCanal[selectedCanal]?.isEditing ? (
                         <>
                           <button
                             onClick={guardarMensaje}
                             disabled={isLoading}
                             className="flex-1 px-4 py-3 bg-green-600 text-white rounded-lg hover:bg-green-700 disabled:opacity-50 flex items-center justify-center space-x-2 text-sm font-medium"
                           >
                         ...
                         <>
                           <button
                             onClick={iniciarEdicion}
                             className="flex-1 px-4 py-3 bg-blue-600 text-white rounded-lg hover:bg-blue-700 flex items-center justify-center space-x-2 text-sm font-medium"
                           >
                           ...
                           <button
                             onClick={restaurarPlantilla}
                             className="flex-1 px-4 py-3 bg-yellow-600 text-white rounded-lg hover:bg-yellow-700 flex items-center justify-center space-x-2 text-sm font-medium"
                           >
                   ...
                   <div className="text-center">
                     <FiMessageSquare className="w-12 h-12 lg:w-16 lg:h-16 text-gray-400 mx-auto mb-3 lg:mb-4" />
